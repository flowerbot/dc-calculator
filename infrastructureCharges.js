...
<script src="/siteassets/scripts/fetch.umd.js"></script>
<script src="/siteassets/scripts/tippy/classListPolyFill.js"></script>
<script src="/prac/infrastructure/siteassets/scripts/svg-inject.min.js"></script>
<script lang="javascript" src="https://cdnjs.cloudflare.com/ajax/libs/xlsx/0.16.3/xlsx.full.min.js"></script>
<script src="https://cdnjs.cloudflare.com/ajax/libs/FileSaver.js/1.3.8/FileSaver.js"></script>
<script src="/prac/infrastructure/siteassets/scripts/html-docx.js"></script>
<link href="/prac/infrastructure/siteassets/scripts/intro.js-2.9.3/introjs.min.css" rel="stylesheet" />

<link rel="stylesheet"
href="http://fonts.googleapis.com/css?family=Kalam">
...
function afterLoad() {
   ...
   function doOverrides(row, planNum) {
        if(debug) console.log("doing overides:" + planNum);
        // console.log(row);
            ...
            fetch("http://tscps/prac/infrastructure/_api/web/lists/GetByTitle('Calculator%20Overrides')/items?$filter=Title eq '" + cc + "'", payload)
                ...
                .then(function(response3) {
                    ...
                    if (d) { // CAN BE UNDEFINED, eg CP02 drainage has nothing to override
                        ccToDisable = d.OverridesChargeControl;
...
document.oninput = function(ev) {
  	...
  	if (ev.target.value) { // assume this catches all inputs
  		...
  		if (ev.target.classList.contains("chkZeros")) {
  			thisStage = ev.target.parentNode.parentNode;
    ...
    document.onclick = function(ev) {
    	if (ev.target.value) { // an input
    		...
    		if (ev.target.classList.contains("selectDevType")) {
    			...
    			var stageOptions = ev.target.parentNode.parentNode.parentNode.parentNode.parentNode; // this is 'whole stage'
    			...
    			if (ev.target.classList.contains("btnCalculate")) {
    				...
    				// INITIALISE
    				//*********************************
    				var wholeStage = ev.target.parentNode.parentNode;
    				if(debug) console.log(wholeStage);
    				var stageTotal = 0;
    				var stageDiscount = 0;
    			...
    			if (ev.target.classList.contains("chkClone")) {
    				var numStages = document.querySelectorAll(".wholestage").length;
    				console.log('button clicked');
    				var clone = ev.target.parentNode.parentNode.parentNode.parentNode.cloneNode(true);
    				...
    				ev.target.parentNode.parentNode.parentNode.parentNode.parentNode.appendChild(clone);
    ...
    function calculateTotals(thisRow) {
    	var parentStage = thisRow.parentNode.parentNode.parentNode;
    ...
    fetch("http://tscps/prac/infrastructure/_api/web/lists/GetByTitle('S7.11 Plans')/items?$filter=Current_x0020_Version_x0020_Stat eq 'Adopted'", payload)
    	...
    	.then(function(data) {
    		...
    		// gotta filter out discontinued plans
    		records.sort(sortPlanNums);
    		planList = records;
    		return records;
    	})
    	.then(function(d3) {
    		//   console.log(d3);
    		fetch("http://tscps/prac/infrastructure/_api/web/lists/GetByTitle('Calculation Data')/items", payload)
    			...
    			.then(function(data2) {
    				...
    				for (i = 0; i < planList.length; i++) {
    					var items = calcData.filter(function(item) {
    						return item.PlanNumber == planList[i].Plan_x0020_Number;
    					})
...
function saveStatus(planList, type) {
	...
	var stages = document.querySelectorAll(".wholestage");
	...
	for (i = 0; i < stages.length; i++) {
		...
		for (k = 0; k < planRows.length; k++) {
			var dataObj = planList.filter(function(d) {
				return d.Plan_x0020_Number == planRows[k].getAttribute("data-plannum");
			})
			dataObj1 = dataObj[0];
			...
			for (j = 0; j < levyRows.length; j++) {
				...
				values["planNum"] = dataObj1.Plan_x0020_Number;
				...
				dataObj[dataObj.Plan_x0020_Number] = values;
				var planNum = dataObj1.Plan_x0020_Number;
		...
		.then(function(response3) {
			...
			for (i = 0; i < rateData.length; i++) {
				var matched = tempRateData.filter(obj => {
					//  console.log(obj);
					return obj.Plan_x0020_Number === rateData[i].Plan_x0020_Number && obj.Title === rateData[i].Title && obj.ChargeControl === rateData[i].ChargeControl;
				})
				//  console.log(matched);
				var updateObj = {};
	...
	for (i = 0; i < allPlanRows.length; i++) {
		...
		for (k = 0; k < allLevyRates.length; k++) {
			...
			var rdObj1 = rateData.filter(obj => {
				//  console.log(obj);
				/*   if(rateTitle == obj.Title && planNum == obj.Plan_x0020_Number) 
				   {
				       console.log("match!!!");
				       console.log(rateTitle + ":" + obj.Title);
				   }  */
				return rateTitle == obj.Title && planNum == obj.Plan_x0020_Number && obj.ChargeControl == rateCC;
			});
			// if(!rdObj) 
			rdObj = rdObj1[0];
			...
			if (rdObj.unmatched == false) {
				...
				if(debug) console.log("planlist:")
				if(debug) console.log(planList);
				var cp18obj = findObjectByKey(planList, "Plan_x0020_Number", 18);
				if(debug) console.log(cp18obj);
	...
	fetch("http://tscps/prac/infrastructure/_api/web/lists/GetByTitle('S94%20Levy%20Rates')/items?$filter=Effective le datetime'" + chargeDate.toISOString() + "' and Ended ge datetime'" + chargeDate.toISOString() + "'", payload)
		...
		.then(function(response3) {
			...
			for (k = 0; k < rateData.length; k++) {
				if (rateData[k].Plan_x0020_Number == 4) {
			...
			for (m = 0; m < rateData.length; m++) {
				if (rateData[m].Plan_x0020_Number == 4) {
			...
			for (i = 0; i < planList.length; i++) {
				var rates = rateData.filter(function(rate) {
					return rate.Plan_x0020_Number == planList[i].Plan_x0020_Number;
				})
				var k = 0;
				var j = 0;
				var devTypes = [];
				...
				for (k = 0; k < rates.length; k++) {
					...
					if (rates[k].Applies_x0020_to) {
						for (j = 0; j < rates[k].Applies_x0020_to.results.length; j++) {
							//   console.log(rates[k].Applies_x0020_to.results[j]);
							var newVal = rates[k].Applies_x0020_to.results[j];
							// avoid duplicates
			...
			for (i = 0; i < planList.length; i++) {
				...
				for (k = 0; k < rateArray.length; k++) {
					...
					rateText += "<div class='tr levyRow" + overridden + commStyle + "' data-unit='" + oneUnit + "' data-overrides='" + overRides + "'" + overRideTitle + sectorData + lacData + ">" + "<div class='td rateTitle'>" + rateArray[k].Title + "</div>" + "<div class='td baseRate extra hidden' data-baserate='" + oneValueBaseRate + "'>" + Math.round(oneValueBaseRate * 100) / 100 + " per " + baseRateUnit + "</div>" + "<div class='td rateAmount' " + "data-rate='" + Math.round(oneValue * 100) / 100 + "'" + " data-1ETmultiplier='" + ETconversion + "'" + ">" + Math.round(oneValue * 100) / 100 + " per " + oneUnit + "</div>" + "<div class='td rateInput'><input type='text' class='rateInput' value=0 /> " + oneUnit + "s</div>" + "<div contenteditable=true class='td resInput cap hidden'><span>0</span></div>" + "<div class='td resTotal cap hidden'><span>0</span></div>" + "<div class='td rateTotal'><span>0</span></div>" + "<div class='td rateDiscountDesc discount hidden'><span></span></div>" + "<div class='td rateDiscount discount hidden'><span>0</span></div>" + "<div class='td rateDiscountedTotal discount hidden'><span>0</span></div>" + "<div class='td rateCapAdjust cap hidden'>0</div>" + "<div class='td rateCappedTotal discount'><span>0</span></div>" + "<div class='td rateAppliesTo extra hidden'>" + rateArray[k].Applies_x0020_to.results.toString().replace(/,/g, ', ') + "</div>" + "<div class='td rateCC extra hidden' data-cc='" + rateArray[k].ChargeControl + "'>" + rateArray[k].ChargeControl + "</div>" + "<div class='td rateAT extra hidden'>" + rateArray[k].Autotext + "</div>" + "</div>"
				}
				rateText += "</div>";
				var prefix = "CP" + ("00" + planList[i].Plan_x0020_Number).substr(-2, 2); //pad with 1 zero
				//console.log(prefix);
				var rowLocal = "";
				if (planList[i].local == true) rowLocal = " localPlan";
				html += "<div class='tr planRow inactive" + rowLocal + "' title='" + planList[i].Title + "' " + "data-plannum='" + planList[i].Plan_x0020_Number + "' " + "data-devtypes='" + planList[i].DevTypes.toString() + "' " +
...
function sortPlanNums( a, b ) {
  if ( a.Plan_x0020_Number < b.Plan_x0020_Number){
    return -1;
  }
  if ( a.Plan_x0020_Number > b.Plan_x0020_Number ){
    return 1;
...
/*  from https://autoprefixer.github.io/ */
/* basically all the extra stuff was to try to get it to work for Edge, it didn't, so I removed it) */
.swal2-radio {
...
} /*.swal2-radio > *:nth-child(1) {
    -ms-grid-row: 1;
    -ms-grid-column: 1;
} */


    #btnImport, #btnHints, #btnGuide, #btnComGuide {

        border-radius: 45%;
        outline:none;
      box-shadow: 20px 38px 34px -26px hsla(0,0%,0%,.2);
      border-radius: 255px 15px 225px 15px/15px 225px 15px 255px;
      border:solid 1.5px rgb(160, 149, 126);
...
.actionRow:nth-child(odd) {
    background-color:whitesmoke;
} 

/*
.actionRow.hidden:nth-child(even) + .actionRow:nth-child(odd) + .actionRow:nth-of-type(even),
.actionRow.hidden:nth-child(even) + .actionRow.hidden:nth-child(odd) + .actionRow:nth-of-type(even)
 {
    background-color:whitesmoke;
}
*/
...
.inlineblock {
    ...
    .DevTypeInputs label.lblMultiRes {
        width: 150px;
        text-align: left;
        padding-left: 0px;
    }
    ...
    .DevTypeInputs input {
     ...
     .DevTypeInputs input.txtCap {
         width: 50px;
         text-align: right;
     }

   /*  .DevTypeInputs input#txtTripsRes, 
     .DevTypeInputs input#txtTripsSub, 
     .DevTypeInputs input#txtTripsCom,
     .DevTypeInputs input#txtTripsCom */
     .DevTypeInputs input.trips {
         width: 100px;
         text-align: right;
     }  

     .DevTypeInputs input.discountCheckBox {
         width: auto;
         margin-left: 10px;
     }

     .DevTypeInputs input.txtETsCom {
     ...
     .planContainer .planRow:not(.hidden):nth-child(odd) {
         background: #fff8eb;
     }
...
/*
.planRow,
     .planRow.hidden:nth-child(even) + .planRow:nth-child(odd) + .planRow:nth-of-type(even),
.planRow.hidden:nth-child(even) + .planRow.hidden:nth-child(odd) + .planRow:nth-of-type(even)
 {
    background-color: white;
}

.planRow:nth-child(even),
.planRow.hidden:nth-child(even) ~ .planRow:nth-child(odd),
.planRow.hidden:nth-child(even) + .planRow.hidden:nth-child(odd) ~ .planRow:nth-of-type(even)
 {
    background-color:#fff8eb;
}
*/
     .DevTypeInputs input.otherCreditsDesc {
         width: 150px;
     }
    ...
    .rateDiscountedTotal {
        min-width: 120px; /* a lil wider cos the heading is longer */
    }
    ...
    .infoImage:hover, .infoDiv:hover .infoImage {
   ...
   /* .infoImage > fill {
    ...
    .rateTotal, .rateDiscount, .rateDiscountedTotal, .rateCappedTotal, .resTotal, .resInput, .rateCapAdjust, .wdNumber {
        text-align: right;
        min-width: 100px;
        border-bottom: 1px dotted silver;
        padding-right: 5px;
    ...
    .totalFooter .rowHeading .rateTitle, 
    .totalFooter .rowHeading .rateAmount,
    .totalFooter .rowHeading .resInput
     {
        border-bottom: none;
    }

    .stageGrandTotal, .stageResidentialTotal,.stageCappedTotal, .stageCapAdjust,
     .stageTotalDiscount, .stageDiscountedTotal {
        font-weight:700;
        font-size: larger;
    ...
    #floatableNotes {
        ...
        /* this stopped the note from being vertically draggable */
    /* transition: top 0.5s, visibility 0s linear 0s, opacity 300ms; */
    }

    #floatableNotes ol li {
        text-align: left;
    ...
    .devTypeSelections, .dateAndClone {
        display: block;
        width: 100%;
    }
...
<div id="everything">
<div class="calcLogo">
<img  src="/SiteAssets/TSC_logo_MAIN_CMYK_with_SPLogoWindingRoad.png" />
<h2>Infrastructure Charge Calculation</h2>
</div>
<input type='button' id='btnImport' value='import saved calc' class='dontPrint' />
...
<div contenteditable=true id="calcDescription" class="calcDescription">Enter DA/Description</div>
</div>
<div>
    <div id="instructionsContainer">
        ...
        <img class="infoImage" 
        src="/prac/infrastructure/siteassets/scripts/icons/info-circle-solid.svg" 
        onload="SVGInject(this)"></img>  
...
<div class="DevTypeSelections">
    ...
    <div class="DevTypeRow dontPrint chkCreDiv">
        <div class="infoDiv" data-helpid="9">
        <img class="infoImage"
        src="/prac/infrastructure/siteassets/scripts/icons/info-circle-solid.svg" 
        onload="SVGInject(this)"></img></div>
...
<div id="mainFooter"><input type="button" class='dontPrint' id="btnSave" value="Save to Excel" />
    ...
    <input type="checkbox" class="dontPrint chkSaveSP" id="chkSaveSP" checked="checked" /> <span class="dontPrint">Save to SP ~ </span>
    <div id="versionNum" class="inlineblock"></div><div id="datadate" class="inlineblock"></div><div id="preparedByDiv">Prepared by: <span id="preparedBy"></span></div></div>
<div id="wordOutput" class="hidden"></div>
<script src="/prac/infrastructure/siteassets/scripts/intro.js-2.9.3/intro.min.js"></script>
